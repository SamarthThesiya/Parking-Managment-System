...
public class OtpVerificationActivity extends BaseActivity implements OtpVerificationInterface {
    ...
    public void onOtpSuccess(FirebaseUser firebaseUser) {
        ...
        if (firebaseUser.getDisplayName() == null) {
            ...
            controller.callApi(apis.getRoles(Constants.land_owner), true);
        }
    ...
    public void onOtpError(Exception exception) {
        hideLoading();
        if (((FirebaseAuthInvalidCredentialsException) exception).getErrorCode().equals("ERROR_INVALID_VERIFICATION_CODE")) {
            otp_et.setError("Wrong OTP");
        } else {
            ViewUtils.toastMessage(Objects.requireNonNull(exception.getMessage()), this);
        }
    }
    ...
    @Override
    void init() {
        otp_et = findViewById(R.id.et_otp);
